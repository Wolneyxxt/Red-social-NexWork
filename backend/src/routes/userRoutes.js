...
router.put("/me", auth, async (req, res) => {
  const { name, role, company, bio, avatar, banner, location, website } =
    req.body;
  const { data, error } = await supabase
    .from("profiles")
    .update({ name, role, company, bio, avatar, banner, location, website })
    .eq("id", req.userId)
    .select()
    .single();
  if (error) return res.status(500).json({ msg: error.message });
...
router.put("/me/password", auth, async (req, res) => {
  ...
  const { error } = await adminClient.auth.admin.updateUserById(req.userId, {
...
router.post("/me/languages", auth, async (req, res) => {
  ...
  const { data, error } = await supabase
    .from("languages")
    .insert({ user_id: req.userId, name, level })
    .select()
    .single();
  if (error) return res.status(500).json({ msg: error.message });
  res.status(201).json(data);
...
router.post("/me/certificates", auth, async (req, res) => {
  ...
  const { data, error } = await supabase
    .from("certificates")
    .insert({ user_id: req.userId, title, issuer, date, credential_url, image })
    .select()
    .single();
  if (error) return res.status(500).json({ msg: error.message });
  res.status(201).json(data);
